...
export const GarmentCard: React.FC<GarmentCardProps> = ({
  ...
  return (
    <article
      ...
      aria-label={`${name} — ${t('outfit.wornCount', { count: garment.wornCount })}${garment.favorite ? ` — ${t('garment.favorite')}` : ''}`}
      style={{ '--i': index } as React.CSSProperties}
      className={`animate-card-enter stagger-child fabric-grain bg-[#1B1814] border rounded-2xl overflow-hidden shadow-fabric shadow-fabric-hover transition-all duration-300 group cursor-pointer flex flex-col h-full relative ${
        isSelectedForBuilder
          ? 'ring-2 ring-[#C76B3F] border-[#C76B3F] shadow-lg scale-[1.02]'
          : 'border-[#2A2622] hover:border-[#C76B3F]/60'
      }`}
    >
        <div className="relative aspect-[3/4] w-full overflow-hidden bg-[#0E0C0A] ken-burns-hover">
          <OptimizedImage
        ...
        <button
          ...
          title={t('garment.favorite')}
        >
          <Heart className={`w-3.5 h-3.5 ${garment.favorite ? 'fill-[#C76B3F] animate-badge-pop' : ''}`} />
        </button>
        ...
        {!isSelectableForBuilder && (
          <div className="absolute bottom-3 right-3 z-10 flex gap-1.5 opacity-0 group-hover:opacity-100 transition-opacity">
              <button
                ...
                aria-label={t('garment.logWearAria')}
                className="animate-press bg-[#C76B3F] hover:bg-[#b36138] text-[#0B0A08] px-2.5 py-1 rounded shadow-md font-mono text-xs font-semibold flex items-center gap-1 transition-transform active:scale-95"
                title={t('garment.logWearTitle')}
              >
        ...
        <div className="p-4 flex flex-col gap-1 flex-1 justify-between bg-[#1B1814] z-20">
          ...
          <div className="flex items-center justify-between mt-2 pt-2 border-t border-[#2A2622]">
            <p className="font-mono text-xs text-[#A89B8C] font-medium">
              {t('outfit.wornCount', { count: garment.wornCount })}
            </p>
